...
LOCK_NAME = ".whatdidido.json.lock"


def get_file_lock() -> FileLock:
    """
    Get a file lock for synchronizing access to the JSON files.
    """

    return FileLock(LOCK_NAME, timeout=10)
...
class DataStore:
    ...
    def save_provider_data(
        self, provider: BaseProvider, start_date: datetime.date, end_date: datetime.date
    ) -> int:
        """
